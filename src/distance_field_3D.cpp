...
DistanceField3D::DistanceField3D(int width, int height, int depth) : width(width), height(height), depth(depth), max_distance(0), close_list_header(nullptr) {
...
void DistanceField3D::compute(const vector<tuple<int, int, int>>& points) {
    close_list_header = nullptr;
        ...
        for(int j = 0; j < height; j++) {
            for(int k = 0; k < depth; k++) {
                ...
                // 看是不是close
                    ...
                    // 加入close list
                    insertList(i, j, k, voxel_distance[idx(i, j, k)]);
                }
                else {
    ...
    while(close_list_header != nullptr) {
        ...
        int i = closest->index[0];
        int j = closest->index[1];
        int k = closest->index[2];
        ...
        for(int dir = 0; dir < 6; dir++) {
            ...
            if(voxel_state[idx(x, y, z)] == DONE)
                ...
                if(new_distance < voxel_distance[idx(x, y, z)]) {
                    ...
                    while(current->index[0] != x || current->index[1] != y || current->index[2] != z) {
                        previous = current;
...
void DistanceField3D::compute(const vector<unsigned char>& voxels, int width, int height, int depth) {
    close_list_header = nullptr;
    // 距離值沒必要初始化，主要用state判斷
    // initialize voxel state
    max_distance = 0;
...
void DistanceField3D::insertList(int i, int j, int k, double distance) {
    Close *new_node = new Close{distance, {i, j, k}, nullptr};
...
void DistanceField3D::determineInOut() {
    ...
    for(int j = 0; j < height; j++) {
        for(int k = 0; k < depth; k++) {
            ...
            bool is_out = true;
            for(int i = 0; i < width; i++) {
                if(voxel_distance[idx(i, j, k)] == 0 && isRangeValid(i - 1, j, k) && voxel_distance[idx(i - 1, j, k)] != 0) { // 遇到模型就切換
                    is_out = !is_out;
                    continue;
                }
    ...
    for(int i = 0; i < width; i++) {
        for(int k = 0; k < depth; k++) {
            ...
            bool is_out = true;
            for(int j = 0; j < height; j++) {
                if(voxel_distance[idx(i, j, k)] == 0 && isRangeValid(i, j - 1, k) && voxel_distance[idx(i, j - 1, k)] != 0) { // 遇到模型就切換
                    is_out = !is_out;
                    continue;
                }
                if(!is_out) { // 到內部時先判斷鄰居是不是外部，只要有一個是外部就視為外部
    ...
    for(int i = 0; i < width; i++) {
        for(int j = 0; j < height; j++) {
            ...
            bool is_out = true;
            for(int k = 0; k < depth; k++) {
                if(voxel_distance[idx(i, j, k)] == 0 && isRangeValid(i, j, k - 1) && voxel_distance[idx(i, j, k - 1)] != 0) { // 遇到模型就切換
                    is_out = !is_out;
                    continue;
                }
                if(!is_out) { // 到內部時先判斷鄰居是不是外部，只要有一個是外部就視為外部
